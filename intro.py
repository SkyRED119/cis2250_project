...
def intro():
    ...
    if choice == "1":
        subprocess.run(["python3", "analyzer_q1.py", "processed_inflation.csv", "processed_vote_percentages.csv"], check=True)
    elif choice == "2":
        subprocess.run(["python3", "analyzer_q2.py", "processed_age_turnout.csv", "processed_vote_percentages.csv"], check=True)  
    else:
